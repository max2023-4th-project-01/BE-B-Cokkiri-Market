...
public interface CategoryRepository extends JpaRepository<Category, Long> {
	...
	@Query("SELECT new kr.codesquad.category.dto.response.CategoryResponse(c.id, c.name, c.iconName) FROM Category c WHERE c.name IN (:s0, :s1, :s2) ORDER BY CASE c.name WHEN :s0 THEN 1 WHEN :s1 THEN 2 WHEN :s2 THEN 3 ELSE 4 END")
